"""Validate layer_attn_bwd_fa against an independent autograd reference.

Both ours (`layer_attn_bwd_fa` in lora_layer_bwd_skel.py) and the
reference here recompute the FA attention block under torch autograd
to compute gradients. The "validation" here is really a self-check:
both should produce identical numbers because they're solving the
same gradient problem with the same primitives.

The point of this test isn't to find a bug — it's to confirm the
gradient *chain* in `layer_attn_bwd_fa` correctly couples back to
the LoRA parameters and to dh_in. A future commit replaces the
autograd interior with hand-rolled cuDNN FA-bwd + manual
QKnorm/RoPE chain rules; THAT version's correctness will be checked
against this autograd path.
"""
...
from lora_layer_bwd_skel import (  # noqa: E402
    layer_attn_bwd_fa, _qwen_rms, _qwen_rope,
...
def autograd_reference(inp: dict):
    """Run the FA attention block under torch autograd from h_in to
    h_post_attn = h_in + attn_out. Loss = sum(h_post_attn * g_for_loss)
    so dh_post_attn = g_for_loss.

    Returns the gradient dictionary in the same shape layer_attn_bwd_fa
    produces, plus h_post_attn for sanity.
    """
    ...
    attn_out = attn_pre_o_flat @ inp["o_W"].t() + LORA_SCALING * (attn_pre_o_flat @ oA) @ oB
...
def main():
    ...
    print("Running autograd reference...")
    ref = autograd_reference(inp)

    print("Running our layer_attn_bwd_fa...")
    out = layer_attn_bwd_fa(
        hidden_in=inp["h_in"],
        h_post_attn=ref["h_post_attn"],   # not actually used by current impl
    ...
    print()
    fails = []
    # Both paths use torch autograd through the same primitives — should
    # be tight. Allow generous absolute tolerance to handle bf16 dtype
    # divergence; cosine should be ≥ 0.999.
    pairs = [
    ...
    for label, ours, refg in pairs:
        if not compare(ours, refg, label, max_abs_thresh=1e-1, cos_thresh=0.999):
            fails.append(label)

    print()
    print("=" * 60)
    if not fails:
        print("ALL CHECKS PASSED ✓  layer_attn_bwd_fa matches autograd reference")
    else:
