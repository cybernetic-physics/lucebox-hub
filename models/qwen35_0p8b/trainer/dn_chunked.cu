    ...
    int bd_pos_stride)
{
    ...
    __nv_bfloat16 *state_bf  = bf16_base;                  // [Dk, Dv/2]    bf16  (one half at a time)
    ...
    for (int c = 0; c < n_chunks; c++) {
        ...
        for (int half = 0; half < 2; half++) {
            ...
            // Cast state[:, dv_off..dv_off+Dv/2-1] -> state_bf [Dk, Dv/2].
            ...
            __syncthreads();
            // [C, Dv/2] = k_cd [C, Dk] @ state_bf [Dk, Dv/2]
        ...
        __syncthreads();
        // attn_int [C, Dv] = buf_q [C, Dk] @ state [Dk, Dv]. Two halves.
        ...
        for (int half = 0; half < 2; half++) {
            ...
            wmma_gemm_fp32<C, Dv/2, Dk>(
                ...
                warp, n_warps, false);
            __syncthreads();
            // Cast buf_attn (fp32 [C, Dv/2]) to bf16 in buf_kcd[:, dv_off..]
            for (int idx = tid; idx < C * (Dv/2); idx += nt) {
                int j = idx / (Dv/2);
                int d = idx - j * (Dv/2);
    ...
    int S, int H, cudaStream_t stream)
{
    ...
    constexpr int Dv = DN_VAL_DEFAULT;
    constexpr int C  = DN_CHUNK;
    // Smem (must match kernel's layout above).
    // Match the layout in dn_chunked_fwd_kernel:
