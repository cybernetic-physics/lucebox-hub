...
**Caveat — PyTorch baseline:** HF Qwen3.5-0.8B falls back to a
torch-native DeltaNet implementation on this box (no `fla`
/ `causal-conv1d` installed). That's the default PyTorch path for
this model and the one rl/backend sees today; a `fla`-accelerated
PyTorch would shrink the gap but not by an order of magnitude.
