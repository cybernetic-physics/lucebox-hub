...
### LoRA inference (prefill) — ours vs PyTorch HF

Same LoRA A/B tensors wrapped into HF's `nn.Linear` modules for a
direct wall-time comparison. HF path uses torch-native DeltaNet (no
`fla` / `causal-conv1d` — the default PyTorch path for this model).

| S     | PyTorch HF (ms)  | ours (ms)  | our tok/s    | speedup  |
|:-----:|-----------------:|-----------:|-------------:|---------:|
| 128   |  454.2           |   5.1      | 25,066       | **88.9×** |
| 256   |  387.1           |   7.8      | 32,936       | **49.8×** |
| 512   |  483.8           |  13.7      | 37,250       | **35.2×** |
| 1024  |  531.2           |  27.4      | 37,411       | **19.4×** |

Reproduce: `python3 models/qwen35_0p8b/trainer/bench_lora_inference.py --seq-lens 128 256 512 1024`
