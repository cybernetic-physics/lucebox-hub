...
### Per-layer DN, hybrid routing vs HF + fla (B200)

`bench_vs_fla.py`, hybrid routing on, 2026-04-27:
...
Hybrid wins on 6/10 shape-mode points; ties on 2; loses on 2 (long-S
inference where fla saturates SMs via chunk-parallelism, plus a
routing regression at S=256 train). The chunk-parallelism rewrite is
the structural fix for the long-S losses.
