...
| (B) training step (fwd+bwd+optim)        |  517.0 ms |   517.0 ms |   1.00×   |
| (C) combined RL step = sample + train    | 2461.7 ms |   726.9 ms | **3.39×** |

**End-to-end 3.39× over HF+fla.** All of the win is on the sampling
side: the cuBLAS+graph prefill + cooperative-decode megakernel runs at
305 tok/s sustained vs HF generate at 33 tok/s. The training step is
parity by construction — both sides currently route through fla's
chunked Triton bwd via `LoraMegakernelTrainer`'s hybrid DN router
(commit 239db57).

vs **real SGLang** (typically 2-3× faster than HF generate for
sampling), the same numbers scale to roughly 3-4× sampling and
~1.5-2.5× combined. To clear 3× over an SGLang+fla incumbent we
also need a training-side win (chunk-parallelism rewrite + chunked
CUDA bwd port). See `models/qwen35_0p8b/trainer/PERF_NOTES.md`.
