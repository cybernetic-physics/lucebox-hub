...
prefill with **cuDNN FA-2** for FA-layer attention + cooperative decode
+ 32 generated tokens) vs HF `base.generate(...)` on the same
Qwen3.5-0.8B + LoRA, 2026-04-27:

| prompt P | HF generate | ours      | speedup  | HF tok/s | our tok/s |
|---------:|------------:|----------:|---------:|---------:|----------:|
|    128   |   1152.7 ms |   41.5 ms | **27.8×** |       28 |       772 |
|    256   |   1140.0 ms |   46.7 ms | **24.4×** |       28 |       685 |
|    512   |   1245.6 ms |   55.5 ms | **22.4×** |       26 |       576 |
|   1024   |   1253.6 ms |   75.5 ms | **16.6×** |       26 |       424 |
|   2048   |   1258.8 ms |  118.5 ms | **10.6×** |       25 |       270 |
|   4096   |   1432.8 ms |  247.6 ms | **5.8×**  |       22 |       129 |
|   8192   |   1668.1 ms |  497.1 ms | **3.4×**  |       19 |        64 |
|  16384   |   2169.2 ms |  984.6 ms | **2.2×**  |       15 |        32 |

(`our tok/s` here is **generated** tokens / wall, not prompt + generated.)

**Read:** decisive win across the entire shape sweep. The FA-2 swap
(cuDNN's `_scaled_dot_product_flash_attention` instead of our previous
O(S²) `pf_causal_attn`) cuts long-context wall by 1.4-2.6× and brings
the 16K speedup over HF generate from 1.16× to **2.2×**. Memory
traffic for FA attention is now O(S × D) instead of O(S² × D);
compute is still O(S²) but uses cuDNN's heavily-tuned tensor-core
