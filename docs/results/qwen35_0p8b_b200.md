...
2026-04-27, hybrid DN routing live + prefill-based rollout (Tier 0.1):

| stage                                    | incumbent | ours      | speedup |
|:-----------------------------------------|----------:|----------:|--------:|
| (A) sample 64 tokens (greedy)            | 2103.5 ms |    77.4 ms | **27.18×** |
| (B) training step (fwd+bwd+optim)        |  471.0 ms |   471.0 ms |   1.00×   |
| (C) combined RL step = sample + train    | 2574.5 ms |   548.4 ms | **4.69×** |

**End-to-end 4.69× over HF+fla.** Sampling throughput: 827 tok/s vs HF
generate's 30 tok/s. The training step is parity by construction —
